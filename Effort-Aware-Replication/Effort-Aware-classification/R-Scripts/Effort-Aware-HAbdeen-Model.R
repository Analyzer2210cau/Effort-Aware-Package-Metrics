...
data2<-NULL
data2 <- read.csv("E:\\Research-Articles\\Journal-Software-Technology\\Effort-Aware-Replication\\Effort-Aware-classification\\Dataset\\Eclipse-2.1.csv")
...
data2<-data2[, c("IIPUD", "IIPED", "post", "N", "A", "Ca", "Ce", "I", "D", "pkgKLOC", "IPIC", "IPSC", "PF")] 
...
#=========================================================================================================
EffortAware_HAbdeen_Model<-function(dataset)
{
  ...
  ########################################################################################################
    ...
    for (i in seq_len(n.folds))
    {
      ...
      glm.model <- glm((post>0) ~ N+A+Ca+Ce+I+D+IIPUD+IIPED+IPIC+IPSC+PF,  data=train, family= "binomial")
      ...
      PD[i] <-  (TP[i]) / ( FN[i]+ TP[i])
      PFR[i]<- (FP[i]) / ( FN[i]+ TP[i])
      
      specificity[i] <- (TN[i]) / (TN[i]+FP[i]) 
      
      balance[i]= 1- sqrt((0-PFR[i])^2+(1-PD[i])^2)
...
EffortAware_HAbdeen_Model(dataset)
